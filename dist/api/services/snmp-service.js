...
const snmp_monitoring_1 = require("../../snmp/snmp-monitoring");
class SnmpService {
    repository;
    constructor(repository) {
        this.repository = repository;
    }
    async getAllSnmpSystems() {
        return await this.repository.findAllSnmpSystems();
    }
    async getSnmpSystemById(id) {
    ...
    async testSnmpConnection(systemId) {
        ...
        const snmpMonitor = new snmp_monitoring_1.SnmpMonitor(monitor.config);
        return await snmpMonitor.check();
    }
    async monitorSnmpSystem(systemId) {
        const system = await this.getSnmpSystemById(systemId);
        if (!system.monitors ||
            system.monitors.length === 0 ||
            !system.monitors[0]) {
    ...
    async monitorAllSnmpSystems() {
        ...
        for (const batch of batches) {
            const promises = batch.map(async (system) => {
                try {
                    results[system.id] = await this.monitorSnmpSystem(system.id);
                }
                catch (error) {
    ...
    async getSnmpDashboard() {
        const systems = await this.getAllSnmpSystems();
        const results = await this.monitorAllSnmpSystems();
        const upSystems = Object.values(results).filter((r) => r.status === 'up').length;
        const downSystems = Object.values(results).filter((r) => r.status === 'down').length;
        const warningSystems = Object.values(results).filter((r) => r.status === 'warning').length;
        return {
            summary: {
                ...
                overall_uptime: systems.length > 0
    ...
    async getSystemMetrics(systemId, from, to) {
    ...
    async getSystemLastMetrics(systemId, limit = 50) {
        // Verificar se o sistema SNMP existe
        await this.getSnmpSystemById(systemId);
    ...
    convertToMetrics(systemId, result) {
    ...
    extractMemoryFromOids(values) {
        ...
        if (storageUsed && storageSize) {
            return Math.round((parseInt(storageUsed) / parseInt(storageSize)) * 100);
        }
        return undefined;
    }
    extractTemperatureFromOids(values) {
        // Cisco temperature: 1.3.6.1.4.1.9.9.13.1.3.1.3.1
        const ciscoTemp = values['1.3.6.1.4.1.9.9.13.1.3.1.3.1'];
        if (ciscoTemp)
            return parseInt(ciscoTemp);
        // Generic temperature OID
        const genericTemp = values['1.3.6.1.2.1.99.1.1.1.4'];
        if (genericTemp)
            return parseInt(genericTemp);
        return undefined;
    }
    extractInOctetsFromOids(values) {
    ...
    extractOutOctetsFromOids(values) {
