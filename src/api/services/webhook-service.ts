...
import type { SystemWithWebhook, IncomingWebhookData, WebhookMetrics } from '../types/webhook-types';
...
export class WebhookService {
  ...
  async processIncomingWebhook(systemId: string, webhookData: any): Promise<void> {
    ...
    const processedData: IncomingWebhookData = {
      ...
      status: webhookData.status ? this.validateStatus(webhookData.status) : undefined,
    ...
    if (system.monitors[0]?.config?.expectedFields) {
      this.validateRequiredFields(webhookData, system.monitors[0].config.expectedFields);
    }
    ...
    // Salvar métricas
    const metrics = this.convertToMetrics(systemId, processedData, webhookData);
    ...
    if (processedData.status && processedData.status !== system.status) {
      await systemRepository.updateSystemStatus(systemId, processedData.status);
      ...
      if (processedData.status === 'down') {
        await alertRepository.createAlertForWEBHOOK(
          ...
          processedData.message || `O sistema ${system.name} reportou estado Down via webhook`
        );
      } else if (processedData.status === 'warning') {
        await alertRepository.createAlertForWEBHOOK(
          systemId,
          'Sistema reportou warning',
          processedData.message || `O sistema ${system.name} reportou um warning via webhook`
        );
      }
    }
  ...
  async getIncomingWebhookHistory(
    ...
    return await webhookRepository.getIncomingWebhookHistory(systemId, from, to);
  }
  ...
  async getSystemStatus(systemId: string): Promise<{
    ...
    const totalWebhooksReceived = await webhookRepository.countWebhooksReceived(systemId);
  ...
  private validateRequiredFields(data: any, requiredFields: string[]): void {
    for (const field of requiredFields) {
      if (!(field in data) || data[field] === undefined || data[field] === null) {
        throw new Error(`Campo obrigatório '${field}' não encontrado no webhook`);
      }
    }
  }

  private convertToMetrics(systemId: string, data: IncomingWebhookData, rawData: any): WebhookMetrics {
    return {
      time: data.receivedAt,
      systemId,
      status: data.status ? this.mapStatusToNumber(data.status) : undefined,
