...
export class PingMonitor {
    ...
    private async executePing(): Promise<{
        avgTime: number;
        packetLoss: number;
    }> {
    ...
    private parsePingOutput(
        output: string,
        isWindows: boolean,
    ): { avgTime: number; packetLoss: number } {
        if (isWindows) {
    ...
    private parseWindowsPingOutput(output: string): {
        avgTime: number;
        packetLoss: number;
    } {
    ...
    private parseLinuxPingOutput(output: string): {
        avgTime: number;
        packetLoss: number;
    } {
        const lines = output.split('\n');
